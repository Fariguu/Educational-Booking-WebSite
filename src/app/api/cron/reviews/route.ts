...
export async function GET(request: Request) {
  ...
  try {
    ...
    // Trova lezioni confermate che sono terminate ieri
    const { data: recentLessons, error: fetchErr } = await supabase
      .from('lessons')
      .select('id, student_contact, student_name, professor_id, end_time, professors:professor_id(name, slug)')
      .eq('status', 'confirmed')
    ...
    for (const lesson of recentLessons) {
      if (!lesson.student_contact || !resend) continue;

      // Un piccolo check per evitare spam se ha già recensito (opzionale ma utile)
      // L'ideal è che lo studente possa recensire una volta sola
      
      const siteUrl = process.env.NEXT_PUBLIC_SITE_URL || 'http://localhost:3000'
      const profName = (lesson.professors as any)?.name || 'il tuo insegnante'
        ...
        await resend.emails.send({
          ...
          html: `
            <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 32px; border: 1px solid #e2e8f0; border-radius: 16px;">
              ...
              <p style="color: #374151;">Ciao <strong>${lesson.student_name}</strong>,</p>
